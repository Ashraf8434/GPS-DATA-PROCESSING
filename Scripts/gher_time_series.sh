...
#awk -F, 'NR>1 || ($1 < 2016.052)' $input_file > gher_data_for_bestfit.csv

earthquake_date=2015.39


# Calculate the best-fit lines using gmtregress for each set of data
gmt gmtregress gher_data_for_bestfit.csv -i0,1,4 -Fxm  > gher_fit_north.txt
gmt gmtregress gher_data_for_bestfit.csv -i0,2,5 -Fxm > gher_fit_east.txt
gmt gmtregress gher_data_for_bestfit.csv -i0,3,6 -Fxm > gher_fit_vertical.txt

gmt begin GHER_plot png
 gmt subplot begin 3x1 -Ff35c/50c -A+o-1c/-1.5c -M1c/2c --FONT_ANNOT_PRIMARY=15p,Helvetica -T"GHER"
  # plot the north component
  gmt subplot set 0 -A"Reference latitude: 27.98@.N"
  gmt basemap -R2009/2017/-10/10 -BWrbt -Bxa0f0+l"Time (Year)" -Bya2.5f1+l"North (cm)" --FONT_LABEL=30p,Helvetica-Bold --MAP_TICK_LENGTH_PRIMARY=6p/3p
  gmt plot $input_file -i0,1+s100,4+s100 -Sc0.6c -Gblue -Wthinner -Ey+p2p,grey
  gmt plot gher_fit_north.txt -i0,1+s100 -W3p,red
  gmt plot -W4p,purple << EOF
    $earthquake_date -1000
    $earthquake_date 1000
EOF
  gmt basemap -R2009-01-01/2017-01-01/-10/10 -f0T -Bxa1Yf1o+l"Time (Year)" -Bya2.5f1+l"North (cm)" --FONT_LABEL=30p,Helvetica-Bold --MAP_TICK_LENGTH_PRIMARY=6p/3p

  #plot the east component
  gmt subplot set 1 -A"Reference longitude: 84.27@.E"
  gmt basemap -R2009/2017/-14/14 -BWrbt -Bxa0f0+l"Time (Year)" -Bya4f1+l"East (cm)" --FONT_LABEL=30p,Helvetica-Bold --MAP_TICK_LENGTH_PRIMARY=6p/3p
  gmt plot $input_file -i0,2+s100,5+s100 -Sc0.6c -Gblue -Wthinner -Ey+p2p,grey
  gmt plot gher_fit_east.txt -i0,1+s100 -W3p,brown
  gmt plot -W4p,purple << EOF
    $earthquake_date -1000
    $earthquake_date 1000
EOF
  gmt basemap -R2009-01-01/2017-01-01/-14/14 -f0T -Bxa1Yf1o+l"Time (Year)" -Bya4f1+l"East (cm)" --FONT_LABEL=30p,Helvetica-Bold --MAP_TICK_LENGTH_PRIMARY=6p/3p

  #plot the vertical component
  gmt subplot set -A"Reference ellipsoid height: 299.9 m"
  gmt basemap -R2009/2017/-6/8 -BWrbt -Bxa0f0+l"Time (Year)" -Bya2f1+l"Up (cm)" --FONT_LABEL=30p,Helvetica-Bold --MAP_TICK_LENGTH_PRIMARY=6p/3p
  gmt plot $input_file -i0,3+s100,6+s100 -Sc0.6c -Gblue -Wthinner -Ey+p2p,grey
  gmt plot gher_fit_vertical.txt -i0,1+s100 -W3p,green
  gmt plot -W4p,purple << EOF
    $earthquake_date -1000
    $earthquake_date 1000
EOF
  gmt basemap -R2009-01-01/2017-01-01/-6/8 -f0T -Bxa1Yf1o+l"Time (Year)" -Bya2f1+l"Up (cm)" --FONT_LABEL=30p,Helvetica-Bold --MAP_TICK_LENGTH_PRIMARY=6p/3p
