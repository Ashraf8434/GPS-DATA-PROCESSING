...
awk -F, 'NR>1 || ($1 >= 2016)' $input_file > jir2_data_for_bestfit.csv

earthquake_date=2015.39


# Calculate the best-fit lines using gmtregress for each set of data
gmt gmtregress jir2_data_for_bestfit.csv -i0,1,4 -Fxm  > jir2_fit_north.txt
gmt gmtregress jir2_data_for_bestfit.csv -i0,2,5 -Fxm > jir2_fit_east.txt
gmt gmtregress jir2_data_for_bestfit.csv -i0,3,6 -Fxm > jir2_fit_vertical.txt

gmt begin JIR2_plot png
 gmt subplot begin 3x1 -Ff35c/50c -A+o-1c/-1.5c -M1c/2c --FONT_ANNOT_PRIMARY=20p,Helvetica -T"JIR2 Station Time Series"
  # plot the north component
  gmt subplot set 0 -A"Reference latitude: 27.98°N"
  gmt basemap -R2009/2017/-10/10 -BWrbt -Bxa0f0+l"Time (Year)" -Bya2.5f1+l"North (cm)" --FONT_LABEL=30p,Helvetica-Bold
  gmt plot $input_file -i0,1+s100,4+s100 -Sc0.6c -Gblue -Wthinner -Ey+p2p,grey
  gmt plot jir2_fit_north.txt -i0,1+s100 -W2p,red
  gmt plot -W2p,purple << EOF
    $earthquake_date -1000
    $earthquake_date 1000
EOF
  gmt basemap -R2009-01-01/2018-01-01/-10/10 -f0T -Bxa1Yf1o+l"Time (Year)" -Bya2.5f1+l"North (cm)" --FONT_LABEL=30p,Helvetica-Bold

  #plot the east component
  gmt subplot set 1 -A"Reference longitude: 84.27@~o@~E"
  gmt basemap -R2009/2017/-14/14 -BWrbt -Bxa0f0+l"Time (Year)" -Bya4f1+l"East (cm)" --FONT_LABEL=30p,Helvetica-Bold
  gmt plot $input_file -i0,2+s100,5+s100 -Sc0.6c -Gblue -Wthinner -Ey+p2p,grey
  gmt plot jir2_fit_east.txt -i0,1+s100 -W2p,brown
  gmt plot -W2p,purple << EOF
    $earthquake_date -1000
    $earthquake_date 1000
EOF
  gmt basemap R2009-01-01/2018-01-01/-14/14 -f0T -Bxa1Yf1o+l"Time (Year)" -Bya4f1+l"East (cm)" --FONT_LABEL=30p,Helvetica-Bold

  #plot the vertical component
  gmt subplot set -A"Reference ellipsoid height: 299.9 m"
  gmt basemap -R2009/2017/-6/8 -BWrbt -Bxa0f0+l"Time (Year)" -Bya1f1+l"Up (cm)" --FONT_LABEL=30p,Helvetica-Bold
  gmt plot $input_file -i0,3+s100,6+s100 -Sc0.6c -Gblue -Wthinner -Ey+p0.2p,grey
  gmt plot jir2_fit_vertical.txt -i0,1+s100 -W2p,green
  gmt plot -W2p,purple << EOF
    $earthquake_date -1000
    $earthquake_date 1000
EOF
  gmt basemap R2009-01-01/2018-01-01/-6/8 -f0T -Bxa1Yf1o+l"Time (Year)" -Bya1f1+l"Up (cm)" --FONT_LABEL=30p,Helvetica-Bold
