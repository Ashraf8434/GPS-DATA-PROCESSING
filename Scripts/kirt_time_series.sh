...
#awk -F, 'NR>1 || ($1 < 2016)' $input_file > kirt_data_for_bestfit.csv

earthquake_date=2015.39


# Calculate the best-fit lines using gmtregress for each set of data
gmt gmtregress kirt_data_for_bestfit.csv -i0,1,4 -Fxm  > kirt_fit_north.txt
gmt gmtregress kirt_data_for_bestfit.csv -i0,2,5 -Fxm > kirt_fit_east.txt
gmt gmtregress kirt_data_for_bestfit.csv -i0,3,6 -Fxm > kirt_fit_vertical.txt

gmt begin KIRT_plot png
 gmt subplot begin 3x1 -Ff35c/50c -A+o-1c/-1.5c -M1c/2c --FONT_ANNOT_PRIMARY=15p,Helvetica -T"KIRT"
  # plot the north component
  gmt subplot set 0 -A"Reference latitude: 27.69@.N"
  gmt basemap -R2009/2017/-150/50 -BWrbt -Bxa0f0+l"Time (Year)" -Bya25f1+l"North (cm)" --FONT_LABEL=30p,Helvetica-Bold --MAP_TICK_LENGTH_PRIMARY=6p/3p
  gmt plot $input_file -i0,1+s100,4+s100 -Sc0.6c -Gblue -Wthinner -Ey+p2p,grey 
  gmt plot kirt_fit_north.txt -i0,1+s100 -W3p,red
  gmt plot -W4p,purple << EOF
    $earthquake_date -1000
    $earthquake_date 1000
EOF
  gmt basemap -R2009-01-01/2017-01-01/-150/50 -f0T -Bxa1Yf1o+l"Time (Year)" -Bya25f1+l"North (cm)" --FONT_LABEL=30p,Helvetica-Bold --MAP_TICK_LENGTH_PRIMARY=6p/3p

  #plot the east component
  gmt subplot set 1 -A"Reference longitude: 85.29@.E"
  gmt basemap -R2009/2017/-30/20 -BWrbt -Bxa0f0+l"Time (Year)" -Bya4f1+l"East (cm)" --FONT_LABEL=30p,Helvetica-Bold --MAP_TICK_LENGTH_PRIMARY=6p/3p
  gmt plot $input_file -i0,2+s100,5+s100 -Sc0.6c -Gblue -Wthinner -Ey+p2p,grey
  gmt plot kirt_fit_east.txt -i0,1+s100 -W3p,brown
  gmt plot -W4p,purple << EOF
    $earthquake_date -1000
    $earthquake_date 1000
EOF
  gmt basemap -R2009-01-01/2017-01-01/-30/20 -f0T -Bxa1Yf1o+l"Time (Year)" -Bya4f1+l"East (cm)" --FONT_LABEL=30p,Helvetica-Bold --MAP_TICK_LENGTH_PRIMARY=6p/3p

  #plot the vertical component
  gmt subplot set -A"Reference ellipsoid height: 1277.24 m"
  gmt basemap -R2009/2017/-30/80 -BWrbt -Bxa0f0+l"Time (Year)" -Bya20f1+l"Up (cm)" --FONT_LABEL=30p,Helvetica-Bold --MAP_TICK_LENGTH_PRIMARY=6p/3p
  gmt plot $input_file -i0,3+s100,6+s100 -Sc0.6c -Gblue -Wthinner -Ey+p2p,grey
  gmt plot kirt_fit_vertical.txt -i0,1+s100 -W3p,green
  gmt plot -W4p,purple << EOF
    $earthquake_date -1000
    $earthquake_date 1000
EOF
  gmt basemap -R2009-01-01/2017-01-01/-30/80 -f0T -Bxa1Yf1o+l"Time (Year)" -Bya20f1+l"Up (cm)" --FONT_LABEL=30p,Helvetica-Bold --MAP_TICK_LENGTH_PRIMARY=6p/3p
