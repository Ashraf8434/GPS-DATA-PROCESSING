...
#awk -F, 'NR>1 || ($1 < 2016)' $input_file > nast_data_for_bestfit.csv

earthquake_date=2015.39


# Calculate the best-fit lines using gmtregress for each set of data
gmt gmtregress nast_data_for_bestfit.csv -i0,1,4 -Fxm  > nast_fit_north.txt
gmt gmtregress nast_data_for_bestfit.csv -i0,2,5 -Fxm > nast_fit_east.txt
gmt gmtregress nast_data_for_bestfit.csv -i0,3,6 -Fxm > nast_fit_vertical.txt

gmt begin NAST_plot png
 gmt subplot begin 3x1 -Ff35c/50c -A+o-1c/-1.5c -M1c/2c --FONT_ANNOT_PRIMARY=15p,Helvetica -T"NAST"
  # plot the north component
  gmt subplot set 0 -A"Reference latitude: 27.66@.N"
  gmt basemap -R2014/2017/-160/60 -BWrbt -Bxa0f0+l"Time (Year)" -Bya25f1+l"North (cm)" --FONT_LABEL=30p,Helvetica-Bold --MAP_TICK_LENGTH_PRIMARY=6p/3p
  gmt plot $input_file -i0,1+s100,4+s100 -Sc0.6c -Gblue -Wthinner -Ey+p2p,grey
  gmt plot nast_fit_north.txt -i0,1+s100 -W2p,red
  gmt plot -W2p,purple << EOF
    $earthquake_date -1000
    $earthquake_date 1000
EOF
  gmt basemap -R2014-01-01/2017-01-01/-160/60 -f0T -Bxa1Yf1o+l"Time (Year)" -Bya25f1+l"North (cm)" --FONT_LABEL=30p,Helvetica-Bold --MAP_TICK_LENGTH_PRIMARY=6p/3p

  #plot the east component
  gmt subplot set 1 -A"Reference longitude: 85.32@.E"
  gmt basemap -R2014/2017/-50/20 -BWrbt -Bxa0f0+l"Time (Year)" -Bya10f1+l"East (cm)" --FONT_LABEL=30p,Helvetica-Bold --MAP_TICK_LENGTH_PRIMARY=6p/3p
  gmt plot $input_file -i0,2+s100,5+s100 -Sc0.6c -Gblue -Wthinner -Ey+p2p,grey
  gmt plot nast_fit_east.txt -i0,1+s100 -W2p,brown
  gmt plot -W2p,purple << EOF
    $earthquake_date -1000
    $earthquake_date 1000
EOF
  gmt basemap -R2014-01-01/2017-01-01/-50/20 -f0T -Bxa1Yf1o+l"Time (Year)" -Bya10f1+l"East (cm)" --FONT_LABEL=30p,Helvetica-Bold --MAP_TICK_LENGTH_PRIMARY=6p/3p

  #plot the vertical component
  gmt subplot set -A"Reference ellipsoid height: 1278.0764 m"
  gmt basemap -R2014/2017/-40/60 -BWrbt -Bxa0f0+l"Time (Year)" -Bya20f1+l"Up (cm)" --FONT_LABEL=30p,Helvetica-Bold --MAP_TICK_LENGTH_PRIMARY=6p/3p
  gmt plot $input_file -i0,3+s100,6+s100 -Sc0.6c -Gblue -Wthinner -Ey+p2p,grey
  gmt plot nast_fit_vertical.txt -i0,1+s100 -W2p,green
  gmt plot -W2p,purple << EOF
    $earthquake_date -1000
    $earthquake_date 1000
EOF
  gmt basemap -R2014-01-01/2017-01-01/-40/60 -f0T -Bxa1Yf1o+l"Time (Year)" -Bya20f1+l"Up (cm)" --FONT_LABEL=30p,Helvetica-Bold --MAP_TICK_LENGTH_PRIMARY=6p/3p
